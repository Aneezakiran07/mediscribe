...
class PatientRepository {
  ...
  // SAVE DRAFT — partial save, any screen in the flow 
  // All params nullable — saves whatever has been filled so far.
  // Sets status = 'draft' so the records screen can show a badge.
  static Future<String> saveDraft({
    ...
    final cc = history?.complaints.isNotEmpty == true
        ? history!.complaints.first : '';
    ...
    final session = PatientSession()
      ...
      ..patientAge      = patient.age?.toString() ?? ''
      ..patientGender   = patient.gender
      ..patientId       = patient.patientId.isNotEmpty
          ? patient.patientId
          : 'MR-${DateTime.now().millisecondsSinceEpoch.toString().substring(7)}'
      ..modeOfAdmission = patient.modeOfAdmission
      ..dateOfAdmission = patient.dateOfAdmission ?? DateTime.now()
      ..chiefComplaint  = cc
      ..provisionalDx   = ''
      ..status          = 'draft'
      ..patientInfoJson = jsonEncode(_serializePatientInfo(patient))
      ..historyJson     = history    != null ? jsonEncode(_serializeHistory(history))        : '{}'
      ..systemicJson    = systemic   != null ? jsonEncode(_serializeSystemic(systemic))      : '{}'
      ..vitalsJson      = vitals     != null ? jsonEncode(_serializeVitals(vitals))          : '{}'
      ..labsJson        = labs       != null ? jsonEncode(_serializeLabs(labs))              : '{}'
      ..examinationJson = examination!= null ? jsonEncode(_serializeExamination(examination)): '{}'
      ..soapSubjective  = ''
      ..soapObjective   = ''
      ..soapAssessment  = ''
      ..soapPlan        = ''
      ..soapGeneratedAt = DateTime.now();
